...
| 항목 | 설명 |
|---|---|
| **3D 깊이감** | 코드 컬럼이 Z축으로 분포 → DoF(피사계 심도) + 안개로 원근 표현 |
| **몰입 카메라** | 마우스 스크롤 → 종단 이동 / 마우스 이동 → 시차 틸트 (스프링 댐핑) |
| **리드 글리터** | 머리 글자가 프레임마다 랜덤하게 환하게 깜빡임 |
| **컬럼 글리치** | 컬럼 전체가 짧게 2배 밝아지며 펄스 |
| **팔레트 전환** | `1` / `2` / `3` `4` 키로 green / amber / cyan / violet 즉시 전환 |
| **코나미 이스터에그** | `↑↑↓↓←→←→BA` → 무지개 팔레트가 0.5초마다 회전 |
| **미스트 입자** | 300개의 희미한 드리프트 점 (가산 블렌딩) |
| **크로마틱 어버레이션** | 밝은 픽셀 가장자리의 미세한 RGB 분리 (ShaderPass) |
| **적응형 품질** | FPS가 47 미만으로 떨어지면 인스턴스 수가 70%로 자동 감소 |
| **Bloom + DoF + OutputPass** | 5-pass EffectComposer 체인 |
| **제로 의존성** | 모든 코드를 단일 HTML에 인라인 — CDN 한 줄로 즉시 실행 |
...
| **Space** | 일시정지 / 재개 |
| **R** | 카메라 위치 리셋 |
| **1 / 2 / 3 / 4** | green / amber / cyan / violet 팔레트 |
| **H** 또는 우상단 × | 컨트롤 가이드 표시/숨김 |
| **컨트롤 가이드** | 페이지 하단 중앙에 표시 (새 탭마다 기본 표시) |
